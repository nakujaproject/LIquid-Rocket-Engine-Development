...
// Arduino Mega interrupt pins are 2,3,18,19,20,21 (20&21 aren't available to use for interrupts when being used for I2C communication
...
int samplingFrequency_Hz = 2000; // 2000Hz(2KHz) translates to Period of 0.5ms (0.0005s)

int samplingPeriod_ms = 1000/samplingFrequency; // 0.5ms -> 2KHz (This is a stable rate to loop the sensors. Higher f gives less accurate Q values)
...
void loop() {
  ...
  if(GetDataBool==true){
    ...
    doc["Q FUEL"] = flowrateFUEL; // amountFUEL; // flowrateFUEL
...
int SensorsTest(){
  ...
  while(x<3 && FinalBool!=true){ // x<5 && FinalBool!=true
    if((millis() - lastTime1) >= samplingPeriod_ms){
...
//
int SensorData(){
  if((millis()-lastTime2) >= samplingPeriod_ms){
...
int Function_flowmeters(int pulFUEL, int pulGN2, int pulGOX){
  FlowTotalBool = false;
  // 
  float Calibration_factor = samplingPeriod_ms*(0.00593334);
